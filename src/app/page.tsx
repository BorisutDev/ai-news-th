      ...
      <header className="border-b border-white/10 sticky top-0 z-50 bg-[#0a0a0f]/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm text-white/60">
            ...
            <Link href="/" className="hover:text-white transition-colors">ปัญญาประดิษฐ์</Link>
            <Link href="/" className="hover:text-white transition-colors">เทคโนโลยี</Link>
            <Link href="/" className="hover:text-white transition-colors">Startup</Link>
          </nav>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-14">
          ...
          <Link href={`/article/${featured.slug}`} className="lg:col-span-2 group relative overflow-hidden rounded-2xl bg-[#111118] border border-white/10 hover:border-cyan-400/40 transition-all duration-300">
            <div className="h-72 lg:h-80 bg-gradient-to-br from-blue-900/40 via-purple-900/20 to-cyan-900/30 relative overflow-hidden flex items-center justify-center">
              <div className="text-[120px] leading-none opacity-30">{featured.emoji}</div>
          ...
          <div className="flex flex-col gap-4">
            {secondary.map((a) => (
              <Link key={a.slug} href={`/article/${a.slug}`} className="group flex gap-4 p-4 rounded-xl bg-[#111118] border border-white/8 hover:border-cyan-400/30 transition-all">
                <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-blue-900/60 to-purple-900/60 flex items-center justify-center text-2xl shrink-0">{a.emoji}</div>
        ...
        {/* Adsense placeholder */}
        <div className="w-full h-20 rounded-xl border border-dashed border-white/20 flex items-center justify-center mb-14 bg-white/2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rest.map((a) => (
              <Link key={a.slug} href={`/article/${a.slug}`} className="group rounded-xl bg-[#111118] border border-white/8 hover:border-cyan-400/30 overflow-hidden transition-all">
                <div className="h-36 bg-gradient-to-br from-blue-900/40 to-purple-900/30 flex items-center justify-center text-5xl relative">
                  {a.emoji}
                  <div className="absolute top-3 left-3">
                    <span className="bg-white/10 text-white/70 text-xs px-2 py-0.5 rounded-full">{a.category}</span>
                  </div>
                </div>
                <div className="p-4">
